...
| 6 | CPU/GPU pixel drift — measured 0.5 px/axis (0.71 px diagonal) | GPU sampling grid is offset +0.5 px per axis vs CPU (Vulkan texel-center convention). Measured at v6 by impulse-PSF centroid: CPU centroid lands exactly on the integer pixel, GPU on +0.5 px; constant across Size 8–80; identical in NUKE and Flame; OFX output is bit-identical to the NDK plugin per backend (<0.001 px). Root cause is the shared kernel: integer coords are normalized as `coords/resolution` with no `+0.5` (`opendefocus-kernel/src/lib.rs:63`), and the CPU path (`opendefocus-shared/src/cpu_image.rs:85`) denormalizes without it, so the two backends bilerp neighbor pairs offset by one texel. Not OFX-fixable without modifying upstream. See `references/ki6_drift_measurement/MEASUREMENT_RESULT.md` |
...
| 27 | Depth-mode scanline displacement on sparse high-contrast features | In Depth mode, an isolated high-contrast in-focus feature on a sharply different-depth background (e.g. single-pixel dots / thin lines with gaps) can have its scanline written to the wrong output row by the upstream kernel convolution (`opendefocus-kernel::global_entrypoint` / `write_texture`), losing the source row and depositing energy ~N rows away. Triggered by the larger Depth-mode buffer margin (`margin = ceil(maxSize)+1`, vs `ceil(size)+1` in 2D) making the processing buffer taller than the declared resolution; intermittent by `maxSize` value and feature/row alignment. **Dense/real footage is unaffected** (confirmed: dense content + depth ramp renders correctly). Not fixable OFX-side (resolution-match, padding, and non-negative-coordinate workarounds all tested, none avoid it). Deterministic repro: `references/depth_displacement_bug/`. Reproduces in the upstream core; NDK shows a different anomaly on the same pathological input. Not fixable from OFX side |
