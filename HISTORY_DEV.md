...
- **Phase 4 (Defocus 一般 + Advanced)**: 完了 — UAT 待ち
...
| Non-uniform エフェクト未実装 | 段階的追加方針 | 次フェーズで追加 |
| Bokeh Noise 未実装 | 段階的追加方針 | 次フェーズで追加 |
| 残りパラメータ未実装 | 段階的追加方針 | 次フェーズで追加 |

### 次のステップ

1. Phase 4 UAT
2. 追加パラメータ（Camera モード、Non-uniform エフェクト、Bokeh Noise 等）
3. getRegionsOfInterest 実装（タイリング対応）
4. GPU レンダリング対応
5. ピクセルドリフト調査（upstream NDK版 GPU レンダリング）
