...
- Bokeh Shape パラメータ群 (Ring Color〜Aspect Ratio): Filter Type=Disc/Blade の時のみ有効
- Blades/Angle/Curvature: Filter Type=Blade の時のみ有効
...
- **Phase 2 (FFI ブリッジ)**: 完了
- **Phase 2 UAT (NUKE 16.0 / Flame 2026)**: 完了 — FAIL 項目なし
- **Phase 3 (Quality + Bokeh パラメータ)**: 完了 — ビルド成功、UAT 待ち
...
| NDK版とのピクセルドリフト | 座標系差異 (pixel center offset) | upstream 調査 |
| 残り 50+ パラメータ未実装 | 段階的追加方針 | 次フェーズで追加 |

### 次のステップ

1. Phase 3 UAT（Quality / Bokeh パラメータの動作検証）
2. 追加パラメータ（残り 50+ パラメータ）
3. getRegionsOfInterest 実装（タイリング対応）
4. GPU レンダリング対応
5. ピクセルドリフト調査（upstream NDK版の座標系）
