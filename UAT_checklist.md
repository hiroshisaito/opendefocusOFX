...
| # | テスト項目 | 合否 | 備考 |
|---|-----------|------|------|
| 32.1 | HD (1920×1080) GPU レンダリングで既存結果と同等の出力 | RETEST | main ブランチとの A/B 比較。ビルドシステム修正後に再テスト必要 |
| 32.2 | HD (1920×1080) CPU レンダリングで既存結果と同等の出力 | RETEST | Use GPU=false で比較。ビルドシステム修正後に再テスト必要 |
...
| 32.5 | Quality=Low (stripe 256px) で正常にレンダリング | RETEST | ストライプ境界に継ぎ目がないこと。ビルドシステム修正後に再テスト必要 |
| 32.6 | Quality=Medium (stripe 128px) で正常にレンダリング | RETEST | ストライプ境界に継ぎ目がないこと。ビルドシステム修正後に再テスト必要 |
| 32.7 | Quality=High (stripe 64px) で正常にレンダリング | RETEST | ストライプ境界に継ぎ目がないこと。ビルドシステム修正後に再テスト必要 |
| 32.8 | Mode=2D（Depth 未使用）でストライプ分割が正常動作する | RETEST | Depth なしの場合。ビルドシステム修正後に再テスト必要 |
| 32.9 | Mode=Depth + RenderResult=Focal Plane Setup (stripe 32px) で正常描画 | RETEST | 最小ストライプ高さ。ビルドシステム修正後に再テスト必要 |
