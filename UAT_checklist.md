...
## 32. ストライプベースレンダリング（パフォーマンス最適化 Phase 1）

| # | テスト項目 | 合否 | 備考 |
|---|-----------|------|------|
| 32.1 | HD (1920×1080) GPU レンダリングで既存結果と同等の出力 | RETEST | NDK オリジナル版との比較、または Quality 間 (Low/Medium/High) の相互比較で出力一致を確認 |
| 32.2 | HD (1920×1080) CPU レンダリングで既存結果と同等の出力 | RETEST | Use GPU=false。NDK オリジナル版との比較、または Quality 間の相互比較 |
| 32.3 | UHD (3840×2160) GPU レンダリングが成功する | PASS | 以前は wgpu バッファ上限超過で失敗していた | UHDでGPUレンダリング可能になりました。
| 32.4 | UHD (3840×2160) CPU レンダリングが実用的な速度で完了する | PASS | ハング・極端な遅延がないこと |
| 32.5 | Quality=Low (stripe 256px) で正常にレンダリング | PASS | ストライプ境界に継ぎ目がないこと。ビルドシステム修正後に再テスト必要 |
| 32.6 | Quality=Medium (stripe 128px) で正常にレンダリング | PASS | ストライプ境界に継ぎ目がないこと。ビルドシステム修正後に再テスト必要 |
| 32.7 | Quality=High (stripe 64px) で正常にレンダリング | PASS | ストライプ境界に継ぎ目がないこと。ビルドシステム修正後に再テスト必要 |
| 32.8 | Mode=2D（Depth 未使用）でストライプ分割が正常動作する | PASS | Depth なしの場合。ビルドシステム修正後に再テスト必要 |
| 32.9 | Mode=Depth + RenderResult=Focal Plane Setup (stripe 32px) で正常描画 | PASS | 最小ストライプ高さ。ビルドシステム修正後に再テスト必要 |
| 32.10 | Filter Type=Image でストライプ間のフィルタが一貫して適用される | RETEST | カスタム Bokeh 画像入力との併用。ビルドシステム修正後に再テスト必要 |
| 32.11 | Catseye Enable=true でストライプ境界が連続的（継ぎ目なし） | RETEST | 非均一ボケのストライプ境界検証。ビルドシステム修正後に再テスト必要 |
| 32.12 | Barndoors Enable=true でストライプ境界が連続的（継ぎ目なし） | RETEST | 非均一ボケのストライプ境界検証。ビルドシステム修正後に再テスト必要 |
| 32.12a | Astigmatism Enable=true でストライプ境界が連続的（継ぎ目なし） | RETEST | 非均一ボケのストライプ境界検証。グローバル座標修正後に再テスト必要 |
| 32.13 | プロキシモード (1/2, 1/4) で renderScale が正しく反映される | RETEST | ストライプ分割との組み合わせ。ビルドシステム修正後に再テスト必要 |
| 32.14 | レンダリング中のキャンセル（abort）がストライプ間で即時反映される | RETEST | ストライプ間 abort チェック。ビルドシステム修正後に再テスト必要 |
| 32.15 | 極端なボケサイズ (Size=500+) でパディングが大きくてもクラッシュしない | PASS | パディング > ストライプ高さのケース | レンダリング出力に時間はかかるがクラッシュはせず処理を終えた
| 32.16 | GPU 失敗時の CPU フォールバックがストライプループ内で正常に動作する | FAIL | 最初のストライプで GPU 失敗 → 残りを CPU で処理 | 10K以上でもGPUレンダリングできてしまい、テストできなかった。12Kあたりで「Asked for too-large ... image input」というエラーになりレンダリングエラーになる(NUKE)
| 32.17 | マルチフレームレンダリング（Flipbook/Write）でストライプ分割が安定動作する | RETEST | フレーム間でクラッシュ・データ混在がないこと。ビルドシステム修正後に再テスト必要 |
| 32.18 | Flame でストライプベースレンダリングが正常に動作する | RETEST | NUKE と同等の結果。ビルドシステム修正後に再テスト必要 |
